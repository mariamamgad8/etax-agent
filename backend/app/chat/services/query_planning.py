...
def authorize_plan(plan: QueryPlan, context: dict) -> dict:
    ...
    own text. Returns one of:
        ...
        {"decision": "proceed", "companies": [...], "fields": [...],
         "metrics": [...]}             # ready for SQL generation
    """
    ...
    # directly from context, never touch SQL generation at all.
    only_ownership_fields = all(f in OWNERSHIP_FIELDS for f in requested_fields)
    if not requested_metrics and (plan.wants_profile_info or (only_ownership_fields and (requested_fields or not explicit_targets))):
    ...
    if explicit_targets:
        ...
        return {"decision": "proceed", "companies": target_companies, "fields": requested_fields, "metrics": requested_metrics}
    ...
    # only ever applies to majority holdings) rather than erroring.
