...
const STEPS = ['Camera ready', 'Face captured', 'Liveness check', 'Face enrolled'];

function errorMessage(err) {
  if (!(err instanceof ApiError)) return 'Enrollment failed. Try again.';
  if (err.status === 422) return 'No face detected. Position your face inside the frame and try again.';
  if (err.status === 403) return 'Liveness check failed. This looks like a photo or screen rather than a live face.';
  if (err.status === 409) return 'A face is already enrolled for this account.';
  return err.message || 'Enrollment failed. Try again.';
}
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--surface-page)', display: 'flex', flexDirection: 'column' }}>
      <header style={{ display: 'flex', alignItems: 'center', height: 72, padding: '0 var(--space-8)', background: 'var(--white)', borderBottom: '1px solid var(--border-subtle)' }}>
        <Logo height={30} src={LOGO_SRC} subtitle="Face enrollment" />
      ...
      <main style={{ flex: 1, display: 'flex', justifyContent: 'center', padding: 'var(--space-10) var(--space-4)' }}>
        <div style={{ width: '100%', maxWidth: 860 }}>
          <h1 style={{ fontSize: 'var(--text-h1)', marginBottom: 'var(--space-2)' }}>Enrol your face</h1>
          <p style={{ fontSize: 'var(--text-body-md)', color: 'var(--text-body)', marginBottom: 'var(--space-6)' }}>
            Look straight at the camera in even lighting. This is used only to confirm it is you at sign-in.
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.4fr) minmax(0,1fr)', gap: 'var(--space-6)', background: 'var(--white)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-sm)', padding: 'var(--space-6)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
              ...
              {failure ? (
                <Alert tone="danger" title="Enrollment failed">{failure}</Alert>
              ) : done ? (
                <Alert tone="success" title="Face enrolled">Opening your assistant.</Alert>
              ) : cameraStatus === 'denied' ? (
                ...
                <div style={{ display: 'flex', gap: 'var(--space-3)', alignItems: 'flex-start', padding: 'var(--space-4)', background: 'var(--gray-50)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-sm)' }}>
                  ...
                  <p style={{ fontSize: 'var(--text-body-sm)', color: 'var(--gray-800)' }}>
                    {cameraStatus === 'ready' ? 'Position your face inside the frame, then capture.' : 'Starting camera…'}
                  </p>
                </div>
              )}
              <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
                {!done && (
                  <Button fullWidth disabled={cameraStatus !== 'ready' || submitting} onClick={capture}>
                    {submitting ? 'Checking…' : failure ? 'Try again' : 'Capture and enrol'}
                  </Button>
                )}
