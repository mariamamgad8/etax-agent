...
export function AppHeader({ logoSrc = '/assets/etax-logo.png', subtitle = 'Tax Assistant', user, verified, nav, onSignOut, style, ...rest }) {
  return (
    <header {...rest} style={{
      ...
      <Logo height={28} src={logoSrc} subtitle={subtitle} />
      {nav && <nav style={{ display: 'flex', gap: 'var(--space-5)', flex: 1 }}>{nav}</nav>}
      <div style={{ marginLeft: nav ? 0 : 'auto', display: 'flex', alignItems: 'center', gap: 'var(--space-4)' }}>
        {verified && <Badge tone="success" style={{ whiteSpace: 'nowrap' }}><Icon name="shield-check" size={12} /> Identity verified</Badge>}
        {user && (
          <span style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', paddingLeft: 'var(--space-4)', borderLeft: '1px solid var(--border-subtle)' }}>
